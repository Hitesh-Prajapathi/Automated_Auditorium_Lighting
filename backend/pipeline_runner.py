...
async def run_full_context_pipeline(
    ...
    try:
        ...
        if model and model != "ollama/local":
            ...
            # Reset client so it picks up any changes
            fc_proc._client = None
        ...
        try:
            ...
            if ext == ".pdf":
                ...
                acq_result = await asyncio.to_thread(acquire_text, filepath)
        ...
        llm_result = await asyncio.to_thread(process_full_script, script_text)
        ...
        lighting_states = await asyncio.to_thread(generate_all_lighting, scenes)
        continuity_score = await asyncio.to_thread(calculate_lighting_continuity_score, lighting_states)
...
async def run_pipeline(job_id: str, filepath: str, ws_callback: Callable[[Dict], Coroutine], model: str = None, script_type: str = "theatrical_script"):
    ...
    try:
        ...
        try:
            scene_objects, p1_metadata = await asyncio.to_thread(run_phase_1, filepath, model=model)
        except Exception as e:
        ...
        try:
            analyzed_scenes = await asyncio.to_thread(analyze_all_scenes, full_script_text, scene_objects)
            await ws_callback({
                "phase": 2,
                "status": "running",
            ...
            for i, scene in enumerate(scene_objects):
                ...
                result = await asyncio.to_thread(analyze_emotion, scene_dict)
