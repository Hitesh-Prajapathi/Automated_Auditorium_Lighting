...
class AuditoriumSimulation {
    ...
    renderConsole(state) {
        ...
        if (state.context_window) {
            state.context_window.forEach(cue => {
                ...
                const sceneBadge = cue.scene ? `<span class="cue-scene">${cue.scene}</span>` : '';
                const clickAction = `onclick="sendCommand('JUMP', ${cue.id})"`;
                html += `<div class="cue-item ${activeStart}" ${clickAction}>
                            <span style="opacity:0.6; margin-right:8px;">${cue.id}:</span> 
                            ${cue.text} ${sceneBadge}
                         </div>`;
            });
        }
    ...
    initScene() {
        ...
        this.renderer.setSize(window.innerWidth - 320, window.innerHeight - 300); // Reduce width (sidebar) and height (console)
        ...
        this.camera = new THREE.PerspectiveCamera(50, (window.innerWidth - 320) / (window.innerHeight - 300), 0.1, 100);
    ...
    animate() {
        ...
        Object.values(this.fixtures).forEach(mesh => {
            ...
            mesh.userData.intensity += (mesh.userData.targetIntensity - mesh.userData.intensity) * lerpFactor;
            if (mesh.userData.config.hasRGB) {
                mesh.userData.color.lerp(mesh.userData.targetColor, lerpFactor);
            }
